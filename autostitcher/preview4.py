#!/usr/bin/env python3
"""
Mosaic Viewer — super‑flüssige Preview mit OpenCV (copyTo/addWeighted) + Caching + Export

• Lädt img_{px}_{py}.png (z.B. img_0_0.png) aus einem Ordner
• Vorschau jetzt OHNE große Numpy‑Akkus — stattdessen OpenCV‑Ops pro Tile:
    - Overwrite (sehr schnell): cv2.copyTo mit Maske
    - Alpha‑Blend (schnell):    cv2.addWeighted + cv2.copyTo (maskiert)
• Heavy‑Rechenarbeit (Rotation, ggf. Skalierung) wird gecached und nur bei Änderung
  des Winkels bzw. der Anzeige‑Skalierung neu berechnet.
• Export einzelner Layer (volle Canvas) für enblend/enfuse bleibt erhalten.

Trackbars
  step_x, step_y        – Abstand zwischen Kachel‑Ursprüngen (px)
  h_slope, v_slope      – Drift (px/Zeile bzw. px/Spalte)
  alpha%                – Transparenz für Alpha‑Preview (0..100)
  rot(°x0.1)            – globale Rotation in Zehntelgrad (−30..+30°)
  mode                  – Preview‑Modus: 0=Overwrite (sehr schnell), 1=Alpha‑Blend
  grid                  – Kachelrahmen an/aus
  scale%                – Anzeige‑Skalierung (nur Preview, skaliert Tiles vor dem Einfügen)
  fmt                   – Exportformat: 0=PNG, 1=TIFF
  depth16               – Exporttiefe: 0=8‑bit, 1=16‑bit

Tasten
  q/ESC  – beenden
  s      – aktuelles Previewbild speichern (mosaic_preview.png)
  e/x    – Einzel‑Export für enblend/enfuse in EXPORT_DIR (oder --out Pfad)
  r      – Slopes auf 0 setzen
  t      – Rotation 0°
  a      – Alpha 50%
...
def parse_args():
    ap = argparse.ArgumentParser(description="Fast OpenCV Mosaic Viewer")
...
def ensure_scaled_cache(cache: Cache, scale: float):
    ...
    if scale == 1.0:
        cache.scaled_imgs = cache.rot_imgs
               # Hinweis: nicht kopieren, nur Referenz
...
def render_preview_opencv(tiles: List[Tile], centers, min_x: int, min_y: int,
                          cache: Cache, scale: float, alpha: float,
                          mode: int, draw_grid: bool):
    # Canvas in Anzeigegröße anlegen
    W = max(1, int(round((layout_w := (max(cx for cx, _ in centers) - min_x) + cache.rot_w) * scale)))
    H = max(1, int(round((layout_h := (max(cy for _, cy in centers) - min_y) + cache.rot_h) * scale)))
    disp = np.zeros((H, W, 3), dtype=np.uint8)

    # pro Tile rendern
    for idx, ((px, py), _orig, _fn) in enumerate(tiles):
        cx, cy = centers[idx]
        x0 = int(round((cx - cache.rot_w / 2.0 - min_x) * scale))
        y0 = int(round((cy - cache.rot_h / 2.0 - min_y) * scale))
        ...
        if mode == 0:
            # Overwrite – sehr schnell
            cv2.copyTo(src, msk, roi)
        else:
            # Alpha‑Blend nur dort, wo Maske
            blended = cv2.addWeighted(roi, 1.0 - alpha, src, alpha, 0.0)
            cv2.copyTo(blended, msk, roi)

        if draw_grid:
            cv2.rectangle(disp, (x_start, y_start), (x_end - 1, y_end - 1), (255, 255, 255), 1)
...
def main():
    ...
    win = "Mosaic Viewer (fast OpenCV preview)"
    ...
    cv2.createTrackbar("scale%", win, 50, 300, _noop)    # kleine Defaults = flüssiger
    ...
    print("'mode=0' ist am schnellsten. 'scale%' kleiner macht alles flotter.")
    ...
    while True:
        ...
        state = (step_x, step_y, h_slope, v_slope, alpha, angle, mode, grid_on, scale)
        if state != prev_state:
            ...
            centers = compute_centers(tiles, img_w, img_h, step_x, step_y, h_slope, v_slope, min_px, min_py)
            min_x, min_y, _W, _H = layout_bounds(centers, cache.rot_w, cache.rot_h)

            disp = render_preview_opencv(tiles, centers, min_x, min_y, cache, scale, alpha, mode, grid_on)
        ...
        if k in (ord('e'), ord('x')):
            ...
            export_layers(tiles, img_w, img_h, step_x, step_y, h_slope, v_slope, min_px, min_py, angle, export_dir, use_tiff, depth16)
            print("Export fertig.")
